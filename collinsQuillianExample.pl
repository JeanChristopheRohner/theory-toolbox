...
% The theory below also formalizes the time it takes to make an inference (T) and the probability of the inference (X). 
% Making an A isa C inference from A isa B and B isa C is harder; hence the 0.8 penalty for such inferences. 
% The same goes for the probability of making an A has property P inference from A isa B and B has property P.
% The time to deduce a relation is linearly related to the number of links that have to be transversed (isa links or property links).

% SOURCES
% Collins, A. M. and Quillian, M. R. (1969). Retrieval time from semantic memory. Journal of Verbal Learning and Verbal Behavior, 8, 241–248.
